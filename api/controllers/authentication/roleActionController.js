...
async function updateRoleAction(req, res) {
    if (req.actions.includes('updateRoleAction')) {
        try {
            let newRoleAction = { updatedTime: Date.now(), user: req.userId, ...req.body };
            let updatedRoleAction = await RoleActions.findOneAndUpdate({ _id: req.params.id }, newRoleAction);
            if (!updatedRoleAction) {
                let response = new ResponseModel(0, 'No item found!', null)
                res.json(response);
            }
            else {
                let response = new ResponseModel(1, 'Update banner success!', newRoleAction)
                res.json(response);
            }
        }
        catch (error) {
